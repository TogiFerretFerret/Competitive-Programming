...
void solve(){
    ...
    sort(all(edges), [&](const Edge& x, const Edge& y) -> bool {
        return a[x.v] < a[y.v];
    });

    vector<hashmap<ll, ll>> S(n + 1);
    ...
    for(const Edge& e : edges) {
        ...
        ll current_paths = 1;
        ll required_prev_val = val_v - val_u;

        if (required_prev_val > 0) {
            if (S[u].count(required_prev_val)) {
                current_paths = (current_paths + S[u][required_prev_val]) % MOD;
            }
        }

        total_paths = (total_paths + current_paths) % MOD;

        S[v][val_u] = (S[v][val_u] + current_paths) % MOD;
    }
