...
    int E = l - 2 * n;
    ll total_combinations = nCr(l, 2 * n);
    ll losing_combinations = 0;

    for (int k = 0; k <= E; k += (n + 1)) {
        ll ways_inner = nCr(k + n - 1, n - 1);
        ll ways_outer = nCr(E - k + n, n);
        ll term = (ways_inner * ways_outer) % MOD;
        losing_combinations = (losing_combinations + term) % MOD;
    }

    ll winning_combinations = (total_combinations - losing_combinations + MOD) % MOD;
    out((winning_combinations * 2) % MOD);
}
int main(){
