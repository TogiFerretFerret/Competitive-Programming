...
void solve(){
    ...
    function<pair<int, int>(int)> find = [&](int i) -> pair<int, int> {
        ...
        parent[i] = root.first;
        diff[i] = diff[i] ^ root.second;
        return {parent[i], diff[i]};
    };
    function<bool(int, int, int)> unite = [&](int i, int j, int rel) -> bool {
        ...
        if (u != v) {
            parent[u] = v;
            diff[u] = root_i.second ^ root_j.second ^ rel;
            return true;
        }
        return false;
    };
    forn(i, n) {
        fOrn(j, i + 1, n) {
            ...
            int required_rel = (val1 < val2) ? 0 : 1;
            unite(i + 1, j + 1, required_rel);
        }
    }
    vector<int> s(n);
    forn(i, n) {
