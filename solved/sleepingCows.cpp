// Problem: Problem 1. Sleeping Cows
// Judge: USACO
// URL: https://usaco.org/index.php?page=viewproblem2&cpid=1068
// Memory Limit: 256 MB
// Time Limit: 4000 ms
#include <bits/stdc++.h>
using namespace std;
...
static const uint64_t PIDRNDP = (uint64_t)(CHRONO_RANDOM)*0xbffbffbffULL;
...
void solve(){
    ...
    auto dp = make<ll>(n + 1, 2);
    dp[0][0] = 1;
    
    auto ndp = make<ll>(n + 1, 2);
    ...
    for(auto& e : events) {
        ...
        if (type == 0) { // Cow
            ...
            forn(a, n) {
                if (a > 0) {
                    forn(j, 2) {
                        ndp[a - 1][j] = (ndp[a - 1][j] + 1LL * a * dp[a][j]) % MOD;
                    }
                }
