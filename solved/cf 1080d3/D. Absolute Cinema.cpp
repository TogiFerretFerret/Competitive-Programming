...
void solve(){
    ...
    vector<ll> f = make<ll, 0>(n + 1);
    fOrn(i, 1, n + 1) in(f[i]);

    vector<ll> a = make<ll, 0>(n + 1);

    fOrn(i, 2, n) {
        ll num = f[i - 1] - 2 * f[i] + f[i + 1];
        a[i] = num / 2;
    }
    ...
    fOrn(i, 2, n) {
        sum_f1_known += a[i] * (i - 1);
        sum_fn_known += a[i] * (n - i);
    }

    if (n > 1) {
        a[n] = (f[1] - sum_f1_known) / (n - 1);
        a[1] = (f[n] - sum_fn_known) / (n - 1);
    }

    vector<ll> ans;
    ans.reserve(n);
    fOrn(i, 1, n + 1) ans.pb(a[i]);
