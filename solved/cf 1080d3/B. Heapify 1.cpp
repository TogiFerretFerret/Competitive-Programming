...
template<class T> ostream& operator<<(ostream& o, const vector<T>& v) { for(int i=0; i<(int)v.size(); ++i) o << v[i] << (i==(int)v.size()-1?"":" "); return o; }
...
void solve(){
    ...
    function<int(int)> get_odd_part = [&](int x) -> int {
        while ((x & 1) == 0) {
            x >>= 1;
        }
        return x;
    ...
    fOrn(i, 1, n + 1) {
        ...
        in(val);
        if (possible) {
            int idx_temp = get_odd_part(i);
            int val_temp = get_odd_part(val);
            
            if (idx_temp != val_temp) {
                possible = false;
            }
        }
    }
