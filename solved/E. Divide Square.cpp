...
void solve(){
    ...
    vector<int> bit(MAXC, 0);

    function<void(int, int)> update = [&](int idx, int val) -> void {
        for (; idx < MAXC; idx += idx & -idx) {
            bit[idx] += val;
        }
    };

    function<int(int)> query_idx = [&](int idx) -> int {
        int sum = 0;
    ...
    function<int(int, int)> query = [&](int l, int r) -> int {
        if (l > r) return 0;
        return query_idx(r) - query_idx(l - 1);
    };

    for (const auto& ev : events) {
        if (ev.type == 0) { // ADD
            update(ev.y, 1);
        } else if (ev.type == 2) { // REMOVE
            update(ev.y, -1);
        } else { // QUERY
            ans += query(ev.y, ev.y_high);
        }
    }
