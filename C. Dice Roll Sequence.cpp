...
void solve(){
    ...
    function<bool(int, int)> are_adjacent = [&](int u, int v) -> bool {
        if (u == v) return false;
        if (u + v == 7) return false;
        return true;
    };

    vector<int> dp = make<int, 0>(7);
    ...
    fOrn(i, 1, n) {
        vector<int> next_dp = make<int, INF>(7);
        
        fOrn(curr_v, 1, 7) {
            int cost = (curr_v == a[i]) ? 0 : 1;
            
            int min_prev = INF;
            fOrn(prev_v, 1, 7) {
                if (are_adjacent(curr_v, prev_v)) {
                    if (dp[prev_v] < min_prev) {
                        min_prev = dp[prev_v];
                    }
                }
            }
